...
job_type = st.selectbox("Jenis Pekerjaan",
                        options=["", "Kocor Pupuk", "Jojoh Pupuk" ,"Spray",])
...
if job_type == "Jojoh Pupuk":
    st.header("Form Pemupukan Jojoh")

    conn = st.connection('gsheets', type=GSheetsConnection)
    # st.write(conn)
    existing_data = conn.read(spreadsheet=url,worksheet='pupuk_jojoh', usecols=list(range(7)), ttl=5)
    ...
    dropdown_selection = st.selectbox("Lokasi",
                                      options=blok_katalog_options,
                                      index=None)
    pohon = st.number_input("Jumlah Pohon",
                             min_value=0,
                             max_value=120,
                             step=1)
